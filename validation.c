...
void validation()
{
	...
	if(authentication_level==11)
	{
		...
		break;
		case 2:
		P("Enter item to search (for example, try searching Coffee)\n");
		S("%s",item);
		search(item);
		}		
		break;
...
int getItemNumber(char b[][100],int index,int len)
{
	int i,j,c=0;
	for(i=0;i<index;i++)
	{
...
void show_menu(int hotel_code)
{
	...
	rewind(fp1);
	char buffer1[1000];
	while ((fgets(buffer1, 1000, fp1)) != NULL)
	{
		f=0;
		for(i=0;buffer1[i];i++){
...
int tokenize(char code[],char s[],char buffer[100][100],int index[100],int price[100])
{
	char ch,t;
	char item_details[100][100],b[100][100];
...
void search(char code[])
{
	FILE *fp;
	int k=0,len=0,i;
	int index[100],price[100];
	char ch;
	char items_menu[100][100];
	fp=fopen("restaurant_menu.txt","r");
	char buffer[1000];
	while( ( ch = fgetc(fp) ) != EOF )
	{
...
void add_to_db(char name[],char address[],char item[100][100],char price[100][100],char cuisine[100][100],int k,int p)
{
	...
	for(i=0;i<p;i++){
	ch++;
	form_a_line_menu(details,ch,cuisine[i]);
...
void administrate()
{
	...
	P("Enter 1 to add Restaurants, 2 to display\n");
	S("%d",&ch);
	S("%c",&buffer);
	switch(ch)
	{
